(function(){
  ...
  function show(el,msg){ if(!el) return; if(typeof msg==='string') el.textContent = msg; else el.innerHTML = msg; el.style.display='block'; }
  function hide(el){ if(!el) return; el.style.display='none'; el.textContent=''; el.innerHTML=''; }

  function bind(){
    const f = byId('f');
    if (f) f.addEventListener('submit', DR_submit);
  }
  ...
  window.DR_submit = async function(e){
    ...
    try{
      ...
      if(j && j.ok){
        show(ok,'✅ Alert created! '+(j.created?.length||1)+' area(s). We will email you on changes.');
