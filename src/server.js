...
// Canonical HTTPS + domain redirect (safe)
...
app.post('/api/admin/availability', adminAuth, async (req, res) => {
  try {
    ...
    for (const t of targets) {
      try {
        await sendAvailability(String(t.email).trim(), { practice, postcode: t.postcode, link });
        sent++;
      } catch (e) {
        console.warn('sendAvailability failed for', t.email, e.message);
      }
    }
