...
app.post('/api/watch', limiter, async (req, res) => {
  try {
    ...
    if ((u?.welcomed|0) === 0) { try { await sendWelcome(String(email).trim()); markWelcomed(String(email).trim()); } catch {} }
...
app.get('/api/watches', (req, res) => {
  try { res.json({ items: listWatchesPublic() }); } catch (e) { res.status(500).json({ ok:false, error:'failed' }); }
});
const adminPass = process.env.ADMIN_PASSWORD || '';
const adminAuth = basicAuth({ users: { admin: adminPass || 'unset' }, challenge: true, unauthorizedResponse: 'Auth required' });
app.get('/api/admin/watches', adminAuth, (req, res) => { try { res.json({ items: listWatchesAdmin() }); } catch (e) { res.status(500).json({ ok:false, error:'failed' }); } });
app.post('/api/admin/test-email', adminAuth, async (req, res) => {
  try { const { to } = req.body || {}; if (!to) return res.status(400).json({ ok:false, error:'to required' }); await sendTest(String(to).trim()); res.json({ ok:true }); }
  catch(e){ console.error('[admin test-email]', e); res.status(500).json({ ok:false, error:'failed' }); }
});
app.post('/api/admin/availability', adminAuth, async (req, res) => {
  try {
    const { postcode, practice, link } = req.body || {};
    if (!postcode || !practice) return res.status(400).json({ ok:false, error:'postcode and practice required' });
    const pc = String(postcode).toUpperCase().replace(/\s+/g, '');
    const all = listWatchesAdmin();
    const targets = all.filter(w => (w.postcode || '').startsWith(pc));
    let sent = 0;
    for (const t of targets) { try { await sendAvailability(String(t.email).trim(), { practice, postcode: t.postcode, link }); sent++; } catch(e){ console.warn('sendAvailability failed for', t.email); } }
    res.json({ ok:true, matched: targets.length, sent });
  } catch(e){ console.error('[admin availability]', e); res.status(500).json({ ok:false, error:'failed' }); }
});
...
app.post('/api/checkout/session', async (req, res) => {
  try {
    ...
    const session = await (new Stripe(process.env.STRIPE_SECRET)).checkout.sessions.create({ mode, line_items: [{ price, quantity: 1 }], customer_email: email, success_url: successUrl, cancel_url: cancelUrl, metadata: { plan } });
    res.json({ ok:true, url: session.url });
  } catch (e) { console.error('[checkout]', e); res.status(500).json({ ok:false, error:'stripe failed' }); }
});
