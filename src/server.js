...
import {
  ensureDb, createWatch, listWatchesPublic, listWatchesAdmin,
  upsertUser, getUser, setPlan, markWelcomed
} from './db.js';
...
// Canonical HTTPS + domain redirect
app.set('trust proxy', true);
const PRIMARY = (process.env.PRIMARY_DOMAIN || '').toLowerCase();
const DO_NOT_REDIRECT = new Set(['/webhook', '/health', '/favicon.ico']);
app.use((req, res, next) => {
  try {
    if (!PRIMARY || DO_NOT_REDIRECT.has(req.path)) return next();
    const host = String(req.headers.host || '').toLowerCase();
    const proto = String(req.headers['x-forwarded-proto'] || req.protocol || 'http').toLowerCase();
    if (host === PRIMARY && proto === 'https') return next();
    const url = `https://${PRIMARY}${req.originalUrl || '/'}`;
    const current = `${proto}://${host}${req.originalUrl || '/'}`;
    if (url === current) return next();
...
app.get('/api/watches', (req, res) => {
  ...
  catch { res.status(500).json({ ok:false, error:'failed' }); }
});
...
app.post('/api/admin/availability', adminAuth, async (req, res) => {
  try {
    ...
    for (const t of targets) {
      try { await sendAvailability(String(t.email).trim(), { practice, postcode: t.postcode, link }); sent++; }
      catch (e) { console.warn('sendAvailability failed for', t.email, e.message); }
    }
...
const scanLimiter = rateLimit({ windowMs: 60_000, max: 2 });
app.post('/api/scan', scanLimiter, async (req, res) => {
  ...
  try {
    ...
    res.json({ ok:true, ...r, time: new Date().toISOString() });
  } catch { res.status(500).json({ ok:false, error:'scan failed' }); }
});

// Manual scan stub
app.post('/api/watch/:id/run', (req, res) => {
  res.json({ ok: true, result: { scanned: 0, alerts: 0 } });
});
