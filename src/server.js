...
const REDIRECT_EXCEPT = new Set(['/webhook', '/health']);
app.use((req, res, next) => {
  try {
    if (REDIRECT_EXCEPT.has(req.path) || !PRIMARY) return next();
    const host = String(req.headers.host || '').toLowerCase();
    const proto = (req.headers['x-forwarded-proto'] || req.protocol || 'http').toLowerCase();
    const hostMismatch = host && host !== PRIMARY;
    const isNotHttps = proto !== 'https';
    if (hostMismatch || isNotHttps) {
