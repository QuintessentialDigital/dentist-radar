/**
 * DentistRadar Scanner v7.8.1
 * - Discovery tries (in order):
 *     /find-a-dentist/results/postcode=...&distance=...
 *     /find-a-dentist/results?postcode=...&distance=...
 *     /find-a-dentist/locationsearch/3?postcode=...&distance=...
 * - Matches detail links under /services/dentist/ (singular), also tolerant of plural/variants.
...
async function fetchText(url, tries=0){
  ...
  try{
    const r = await fetch(url, {
      ...
      signal: ctrl.signal,
      headers: {
        "user-agent": "Mozilla/5.0 DentistRadar/7.8.1",
...
function buildDiscoveryUrls(pc, radius){
  ...
  return [
    `${base}/results/postcode=${encPC}&distance=${encR}`,     // the format you confirmed
    `${base}/results?postcode=${encPC}&distance=${encR}`,     // fallback query form
    `${base}/locationsearch/3?postcode=${encPC}&distance=${encR}` // legacy
  ];
}
...
/** Critical: match singular dentist path; allow plural/variants as fallback. */
function extractPracticeLinks(html){
  if (!html) return [];
  const $ = cheerio.load(html);

  // IMPORTANT: this is a single QUOTED string; do not remove quotes.
  ...
  for (const b of bases){
    for (let p=1; p<=MAX_PAGES; p++){
      ...
      for (const u of urls){
        ...
        if (links.length){ all = all.concat(links); }
        else if (p > 1){ break; }
      }
    }
    if (all.length) break;
...
export async function scanPostcode(pc, radiusMiles = 25){
  ...
  console.log(`DentistRadar scanner v7.8.1`);
...
/* -------------- tiny self-test to avoid this regression ---------- */
(() => {
  // If someone removes quotes around the selector below, it will throw here instead of deep in runtime.
  const s = 'a[href*="/services/dentist/"]';
  if (typeof s !== "string" || !s.includes("/services/dentist/")) {
    throw new Error("Selector string corrupted — it must be a quoted string.");
  }
})();
