 ...
 * - Discovers NHS dentist detail URLs from postcode+radius using Chromium (Playwright).
 * - Accepts NHS cookie banner so links actually render.
 * - Supports path-style and query-style results URLs + simple pagination.
 * - Loads Appointments page, applies acceptance logic, emails watchers (Postmark).
 * - Uses only Watch, Postcodes, EmailLog collections (no Practices DB needed).
 *
 * Required ENV:
 *   MONGO_URI
 ...
 *   PW_SLOWMO="0"     (e.g. "250" for visual debugging)
 */
...
/* ─────────────────────────
   Results URLs (path + query styles) + pagination
   ───────────────────────── */
function buildResultsUrls(pc, radiusMiles) {
  ...
  // Simple pagination for each base
...
/* ─────────────────────────
   Playwright discovery helpers
   ───────────────────────── */
async function acceptCookieBanner(page) {
  // Try common NHS cookie acceptors
  const selectors = [
  ...
  try {
    ...
    for (const u of candidates) {
      try {
        ...
        // wait a little for hydration / list
        await page.waitForTimeout(1200);

        // if a “results list” container exists, prefer waiting on it
        const anyList = page.locator('a[href*="/services/dentists/"]').first();
        await anyList.waitFor({ state: "visible", timeout: 2500 }).catch(() => {});

        const firstBatch = await collectDetailLinksOnPage(page);
        firstBatch.forEach(x => urls.add(x));

        // pagination via explicit next link if present
        for (let i = 0; i < 5; i++) {
          const next = page.locator('a[rel="next"], nav a[aria-label="Next"], a:has-text("Next")').first();
        ...
        console.log("[DISCOVERY WARN]", u, "→", e?.message || e);
      }
    }
  } catch (e) {
...
async function scanJob({ postcode, radiusMiles, recipients }) {
  ...
  await Promise.all(detailUrls.map(detailsUrl => limit(async () => {
    ...
    // Distance filter (use details or appointments html)
